namespace Skyline.DataMiner.Library.Common
{
	...
	internal class DmsElement : DmsObject, IDmsElement
	{
		...
		/// <summary>
		///     Duplicates the element. It is advised to provide the <see cref="IDma" /> agent object to avoid introducing extra
		///     system calls while creating a IDma object.
		/// </summary>
		...
		public void Update()
		{
			...
			try
			{
				...
				if (this.UpdateRequired())
				{
					...
					IDma targetDma = new Dma(this.Dms, this.DmsElementId.AgentId);
					bool useAlternateversion = targetDma.IsVersionHigher(Dma.SnmpV3AuthenticationChangeDMAVersion);

					AddElementMessage message = this.CreateUpdateMessage(useAlternateversion);
