namespace Skyline.DataMiner.Library.Common
{
	...
	internal class GeneralSettings : ElementSettings
	{
		...
		internal IDmsAlarmTemplate AlarmTemplate
		{
			...
			set
			{
				...
				if (updateRequired)
				{
					...
					alarmTemplateName = value == null ? String.Empty : value.Name;
					alarmTemplate = value;
				}
			}
		}
		...
		{
			if (alarmTemplate == null && !String.IsNullOrWhiteSpace(alarmTemplateName))
			{
				...
				if (response != null)
				{
					switch (response.Type)
					{
						case AlarmTemplateType.Template:
							alarmTemplate = new DmsStandaloneAlarmTemplate(DmsElement.Dms, response);
							break;
						case AlarmTemplateType.Group:
							alarmTemplate = new DmsAlarmTemplateGroup(DmsElement.Dms, response);
							break;
						default:
