namespace Skyline.DataMiner.Library.Common
{
	...
	public class SnmpV3SecurityConfig : ConnectionSettings, ISnmpV3SecurityConfig
	{
		...
		/// <summary>
		///     Returns the correct Authentication Algorithm based on the DataMiner Version.
		///     https://intranet.skyline.be/DataMiner/Lists/Release%20Notes/DispForm.aspx?ID=24423
		/// </summary>
		/// <param name="authenticationAlgorithm">The Authentication Algorithm string value to get its compatible value from.</param>
		/// <returns>A compatible Authentication Algorithm string value.</returns>
		internal static string GetCompatibleAuthenticationAlgorithmToDma(string authenticationAlgorithm)
		{
			if (authenticationAlgorithm.Equals("HMAC-MD5")) return "MD5";
			if (authenticationAlgorithm.Equals("HMAC-SHA")) return "SHA";
			...
			foreach (ConnectionSetting property in this.ChangedPropertyList)
			{
				switch (property)
				{
					...
					case ConnectionSetting.AuthenticationProtocol:
						...
						if (isCompatibilityIssueDetected)
						{
							portInfo.Parity = GetCompatibleAuthenticationAlgorithmToDma(SnmpV3AuthenticationAlgorithmAdapter.ToSLNetParity(this.authenticationAlgorithm));
						}
						else
						{
