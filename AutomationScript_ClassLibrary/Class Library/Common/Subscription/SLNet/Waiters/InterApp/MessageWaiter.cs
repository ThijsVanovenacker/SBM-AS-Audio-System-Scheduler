namespace Skyline.DataMiner.Library.Common.Subscription.Waiters.InterApp
{
	...
	internal class MessageWaiter : IDisposable
	{
		...
		public MessageWaiter(ICommunication connection, ISerializer serializer, params Message[] commands)
		{
			...
			foreach (var commandGroup in commandsGrouped)
			{
				...
				StartMonitor(connection, serializer, dmaId, eleId, pid);
			}
		}
...
#pragma warning restore S2330 // Array covariance should not be used
		...
		private void StartMonitor(ICommunication connection, ISerializer serializer, int dmaId, int eleId, int pid)
		{
			...
			monitor.Start(change =>
			{
				try
				{
					...
					if (parameterData != null && !String.IsNullOrWhiteSpace(Convert.ToString(parameterData, CultureInfo.InvariantCulture)))
					{
						Message internalReturn = MessageFactory.CreateFromRaw(Convert.ToString(parameterData, CultureInfo.InvariantCulture), serializer);
						if (internalReturn != null && monitoredGuids.Contains(internalReturn.Guid))
						{
